...
export function BatchHistoryTab({ clientId }: BatchHistoryTabProps) {
  ...
  const fetchBatches = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`/api/tools/meta-tag-analyser/batch?clientId=${clientId}`);
      if (res.ok) {
  ...
  const getSourceIcon = (source: BatchScan['source']) => {
    switch (source) {
      case 'sitemap':
  ...
  const getSourceLabel = (source: BatchScan['source']) => {
    switch (source) {
      case 'sitemap':
        return 'Sitemap scan';
      case 'url_list':
  ...
  const getStatusBadge = (status: BatchScan['status']) => {
    switch (status) {
      case 'completed':
        return (
    ...
    <>
      <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
        ...
        <div className="divide-y divide-neutral-100">
          {batches.map((batch) => {
            ...
            // Calculate success rate from deduplicated results
            ...
            return (
              <div
                key={batch._id}
                className="px-4 py-4 hover:bg-neutral-50 transition-colors cursor-pointer"
      ...
      {selectedBatch && (
        <BatchReport
          ...
          status={selectedBatch.status as 'completed' | 'failed' | 'cancelled'}
          results={{
            succeeded: selectedBatch.succeeded,
            failed: selectedBatch.failed,
            skipped: selectedBatch.skipped,
          }}
          averageScore={selectedBatch.averageScore}
          totalUrls={selectedBatch.totalUrls}
          completedAt={selectedBatch.completedAt}
          source={selectedBatch.source}
          sourceUrl={selectedBatch.sourceUrl}
